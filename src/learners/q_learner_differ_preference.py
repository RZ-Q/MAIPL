...
class QLearner_differ_perference:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.mixer_optimiser = RMSprop(
            params=self.mixer_params,
            lr=args.lr,
            alpha=args.optim_alpha,
            eps=args.optim_eps,
        )
        self.q_optimiser = RMSprop(
            params=self.q_params, lr=args.lr, alpha=args.optim_alpha, eps=args.optim_eps
        )
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if self.mixer is not None:
            ...
            chosen_action_q_tot_vals = self.mixer(
                chosen_action_qvals_clone, batch["state"][:, :-1]
            )
            target_max_q_tot_vals = self.target_mixer(
                target_max_qvals_clone, batch["state"][:, 1:]
            )

        # Calculate 1-step Q-Learning targets
        targets = (
            rewards + self.args.gamma * (1 - terminated) * target_max_q_tot_vals
        )  # [bs, ep_len, 1]
        ...
        grad_l_qtot = (
            chosen_action_q_tot_vals.grad.repeat(1, 1, self.args.n_agents) + 1e-8
        )
        grad_l_qi = chosen_action_qvals_clone.grad
        grad_qtot_qi = th.clamp(
            grad_l_qi / grad_l_qtot, min=-10, max=10
        )  # (B,T,n_agents)

        mixer_grad_norm = th.nn.utils.clip_grad_norm_(
            self.mixer_params, self.args.grad_norm_clip
        )
        ...
        q_rewards = self.cal_indi_reward(
            grad_qtot_qi,
            td_error.clone().detach(),
            chosen_action_qvals,
            target_max_qvals,
            indi_terminated,
        )  # (B,T,n_agents)
        ...
        # Calculate 1-step Q-Learning targets
        q_targets = (
            q_rewards_clone + self.args.gamma * (1 - indi_terminated) * target_max_qvals
        )  # (B,T,n_agents)
        ...
        q_mask = (
            batch["filled"][:, :-1].float().repeat(1, 1, self.args.n_agents)
        )  # (B,T,n_agents)
        q_mask[:, 1:] = (
            q_mask[:, 1:]
            * (1 - indi_terminated[:, :-1])
            * (1 - terminated[:, :-1]).repeat(1, 1, self.args.n_agents)
        )
        ...
        # preference loss
        preference_labels = self.script_preferences.produce_labels(
            states, actions, indi_rewards
        )
        preference_loss = self.cal_preference_loss(
            q_rewards, preference_labels, self.args.n_agents
        )
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat(
                "mixer_target_mean", (targets * mask).sum().item() / mask_elems, t_env
            )
            ...
            self.logger.log_stat(
                "q_td_error_abs",
                (masked_q_td_error.abs().sum().item() / q_mask_elems),
                t_env,
            )
            self.logger.log_stat(
                "q_q_taken_mean",
                (chosen_action_qvals * q_mask).sum().item() / (q_mask_elems),
                t_env,
            )
            self.logger.log_stat(
                "q_q_target_mean",
                (q_targets * q_mask).sum().item() / (q_mask_elems),
                t_env,
            )
            self.logger.log_stat(
                "reward_i_mean",
                (q_rewards * q_mask).sum().item() / (q_mask_elems),
                t_env,
            )
        ...
        for i in range(agent_num):
            for j in range(i + 1, agent_num):
                ...
                for k in range(len(labels)):
                    l += self.CEloss(r_i_j[k], labels[k])
    ...
    def cal_indi_reward(
        self, grad_qtot_qi, mixer_td_error, qi, target_qi, indi_terminated
    ):
        # input: grad_qtot_qi (B,T,n_agents)  mixer_td_error (B,T,1)  qi (B,T,n_agents)  indi_terminated (B,T,n_agents)
        grad_td = th.mul(
            grad_qtot_qi, mixer_td_error.repeat(1, 1, self.args.n_agents)
        )  # (B,T,n_agents)
    ...
    def load_models(self, path):
        ...
        self.q_optimiser.load_state_dict(
            th.load(
                "{}/q_opt.th".format(path), map_location=lambda storage, loc: storage
            )
        )
