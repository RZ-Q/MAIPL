...
from utils.model import onehot_from_logits, sample_gumbel, gumbel_softmax_sample, gumbel_softmax
...
class MADDPGMAC:
    ...
    def forward_by_agent(self, ep_batch, t, agent_idx):
        agent_inputs = self._build_inputs(ep_batch, t)
        avail_actions = ep_batch["avail_actions"][:, t]
        agent_outs, self.hidden_states = self.agent(agent_inputs, self.hidden_states)
        agent_outs = agent_outs.view(ep_batch.batch_size, self.n_agents, -1)
    ...
    def parameters_by_agent(self):
        return self.agent.get_param_by_agent()
