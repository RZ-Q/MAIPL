...
def run_sequential(args, logger):
    ...
    args.agent_state_shape = env_info["agent_state_shape"][0]

    # Default/Base scheme
    scheme = {
        "state": {"vshape": env_info["state_shape"]},
        "agent_state": {"vshape": env_info["agent_state_shape"][0], "group": "agents"},
    ...
    pref_mac = mac_REGISTRY[args.pref_mac](buffer.scheme, groups, args)

    # Give runner the scheme
    runner.setup(scheme=scheme, groups=groups, preprocess=preprocess, mac=mac)

    # Learner
    learner = le_REGISTRY[args.learner](mac, buffer.scheme, logger, args, pref_mac)

    # Reward model
    reward_model = RewardModel(args, pref_mac, logger)
    ...
    last_log_T = 0
    model_save_time = 0
    interact_count = args.num_interact
    ...
    while runner.t_env <= args.t_max:
        ...
        episode_batch = runner.run(test_mode=False, reward_model=reward_model)
