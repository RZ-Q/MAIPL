...
parser.add_argument('--use_rtgs', action='store_true', default=False)
# parser.add_argument('--gamma', type=float, default=0.99)
parser.add_argument('--offline_data_dir', type=str, default='/data/user/kouqian/files/MAOfflineDatasets/SMAC410_pref/5m_vs_6m_good_5000_16.pkl')
parser.add_argument('--offline_data_type', type=str, default='good')
parser.add_argument('--offline_episodes', type=int, default=10000)
parser.add_argument('--offline_segment_pairs', type=int, default=5000)
parser.add_argument('--offline_episode_bias', type=int, default=0)
parser.add_argument('--offline_batch_size', type=int, default=256)  # episodes used for offline train
...
import wandb
if args.use_rtgs:
    group = args.algorithm + '-' + 'rtgs' + '-' + args.map_name+ '-' + str(args.offline_segment_pairs) + '-' + args.offline_data_type 
else: 
    group = args.algorithm + '-' + args.map_name+ '-' + str(args.offline_segment_pairs) + '-' + args.offline_data_type
wandb.init(project='offline_samc410', group=group, name=str(args.seed))
...
if __name__ == '__main__':
    ...
    if args.algorithm == 'omiga':
        ...
        offline_tconf = TrainerConfig(max_epochs=1, num_agents=num_agents, num_actions=action_dim, batch_size=args.offline_batch_size,
                                      learning_rate=args.offline_lr, num_workers=0, use_rtgs=args.use_rtgs,
