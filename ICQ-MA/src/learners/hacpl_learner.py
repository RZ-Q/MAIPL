...
class HACPLLearner:
    ...
    def train(self, batch0, batch1, t_env, labels, running_log):
        ...
        agents_order = np.arange(self.n_agents) if self.args.fix_order else np.random.permutation(self.n_agents)
        factor0, factor1 = th.ones_like(mask0), th.ones_like(mask1)
        total_loss = []
        for agent_id in agents_order:
            ...
            old_log_pi_taken0 = th.log(pi_taken0[:,:,agent_id]).detach()
            ...
            old_log_pi_taken1 = th.log(pi_taken1[:,:,agent_id]).detach()


            # # ------------- CPL loss ------------------
            log_pi_taken0 = th.log(factor0 * pi_taken0[:,:,agent_id])
            log_pi_taken1 = th.log(factor1 * pi_taken1[:,:,agent_id])
            ...
            pi_taken0 = th.gather(mac_out0, dim=-1, index=actions0)
            pi_taken0[mask0.repeat(1, 1, self.n_agents).unsqueeze(-1) == 0] = 1.0
            new_log_pi_taken0 = th.log(pi_taken0[:,:,agent_id]).detach()
            ...
            pi_taken1 = th.gather(mac_out1, dim=-1, index=actions1)
            pi_taken1[mask1.repeat(1, 1, self.n_agents).unsqueeze(-1) == 0] = 1.0
            new_log_pi_taken1 = th.log(pi_taken1[:,:,agent_id]).detach()

            factor0 = factor0 * th.exp(new_log_pi_taken0 - old_log_pi_taken0)
            factor1 = factor1 * th.exp(new_log_pi_taken1 - old_log_pi_taken1)
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            running_log.update({
                "loss": (sum(total_loss)/len(total_loss)).item(),
