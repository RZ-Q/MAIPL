...
FLAGS_DEF = define_flags_with_default(
    ...
    ################ dir config
    device='gpu',
    dataset_path='/data/user/kouqian/files/MAOfflineDatasets/SMAC410_pref/2c_vs_64zg_good_5000_16.pkl',
    model_dir="MAPT/MAPT-main/result/model",
)
...
def main(_):
    ...
    #################### set logger dir
    save_dir = FLAGS.logging.output_dir + '/' + FLAGS.env + '/' + FLAGS.task + '/' + str(FLAGS.pref_size)
    ...
    for epoch in range(FLAGS.n_epochs):
        ...
        if epoch % FLAGS.eval_period == 0:
            batch_eval = batch_to_torch(index_batch(pref_dataset, range(0,data_size)), FLAGS.device)
            acc = reward_model.evaluation(batch_eval)
            print("acc:", acc)
        ####################### save model
            if FLAGS.save_model and acc > max_acc:
                max_acc = acc
