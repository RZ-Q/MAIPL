...
class NMR(object):
    ...
    def get_reward_for_offline(self, batch):
        ...
        lstm_preds0, lstm_preds1 = [], []
        for i in range(max_len):
            with torch.no_grad():
                lstm_pred0, _ = self.lstm(his_obs0[:,i:i+max_len], his_act0[:,i:i+max_len])
                lstm_pred1, _ = self.lstm(his_obs1[:,i:i+max_len], his_act1[:,i:i+max_len])
                lstm_preds0.append(lstm_pred0[:, -1])
                lstm_preds1.append(lstm_pred1[:, -1])

        lstm_preds0 = torch.stack(lstm_preds0, dim=1).mean(2)
        lstm_preds1 = torch.stack(lstm_preds1, dim=1).mean(2)

        return lstm_preds0.squeeze(1), lstm_preds1.squeeze(1)
